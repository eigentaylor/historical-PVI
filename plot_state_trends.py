...
for state in states:
    ...
    if subplot_mode:
        if plot_house_margins and house_on_same_plot:
            ...
            for i, v in enumerate(relative_margin):
                axes[1].text(i, v + (1 if v > 0 else -1), utils.lean_str(v), ha='center', va='bottom' if v > 0 else 'top', fontsize=8, color='white')
            ...
            try:
                ...
                params, _ = curve_fit(sine_function, x_indices, relative_margin, p0=[1, 1, 0, z[0]])
                A, B, C, D = params
                ...
                x_dense = np.linspace(min(x_indices), max(x_indices), 500)
